...
describe('website', () => {
  ...
  it('cleans html', () => {
    ...
    login()
    cy.get('div').contains('Write your review').click()
    cy.get('textarea[name="comment"]').type('Hey <a href="https://example.com">Click here</a>')
    cy.get('#rating').get('input[value="5"]').click({force:true})
    cy.get('div').contains('Post').click()
    cy.get('div', {timeout:10000}).contains('<a href="https://example.com">').should('exist')
  })
})